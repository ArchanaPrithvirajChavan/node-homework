...
describe("testing task creation", () => {
  ...
  it("17. The object returned has expected title", () => {
    saveData = saveRes._getJSONData();
    expect(saveData.title).toBe("First Task");
  });

  it("18. The object has correct isCompleted value", () => {
    expect(saveData.isCompleted).toBe(false);
  });

  it("19. The object does not include userId", () => {
...
describe("test getting created tasks", () => {
  ...
  it("20. can't get tasks without user id", async () => {
    expect.assertions(1);
  ...
  it("21. user1 gets 200 status", async () => {
    const req = httpMocks.createRequest({
      method: "GET",
    });
  ...
  it("22. tasks array length is 1", () => {
    saveData = saveRes._getJSONData();
    expect(saveData.tasks.length).toBe(1);
  });

  it("23. first task title is correct", () => {
    expect(saveData.tasks[0].title).toBe("First Task");
  });

  it("24. task does not include userId", () => {
...
describe("test show task", () => {
  ...
  it("26. user1 can retrieve task", async () => {
    const req = httpMocks.createRequest({
      method: "GET",
    });
...
describe("update and delete tasks", () => {
  ...
  it("28. user1 can update task", async () => {
    const req = httpMocks.createRequest({
      method: "PATCH",
  ...
  it("29. user2 cannot update task", async () => {
    expect.assertions(1);
...
expect(res.statusCode).toBe(404);
  ...
  it("32. user1 tasks now return 404", async () => {
    const req = httpMocks.createRequest({
      method: "GET",
    });
